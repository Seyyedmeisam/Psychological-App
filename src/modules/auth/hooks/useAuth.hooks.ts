...
import { getMe, login, logout, register, updateAvatar, updateProfile } from '@/modules/auth/services'
import type {
  AuthResponse,
  AuthUser,
  LoginFormValues,
...
export const useUpdateAvatar = (
  options?: UseMutationOptions<AuthUser, Error, File>,
) => {
  const queryClient = useQueryClient()
...
export const useUpdateProfile = (
  options?: UseMutationOptions<AuthUser, Error, UpdateProfileFormValues>,
) => {
  const queryClient = useQueryClient()
  ...
  return useMutation({
    ...
    onSuccess: async (data, variables, onMutateResult, context) => {
      ...
      toast.success(m.auth_profile_updated())
      await options?.onSuccess?.(data, variables, onMutateResult, context)
    },
    onError: async (error, variables, onMutateResult, context) => {
      toast.error(getApiErrorMessage(error))
